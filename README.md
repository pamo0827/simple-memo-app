...
- **URLまとめて追加**: 複数のURLを一度に追加可能。CSV形式、改行区切り、文章中のURL自動抽出に対応
...
### セキュリティ対策
すべてのAPI routesで包括的なセキュリティ対策を実装：

#### 認証・認可
- **認証トークン検証**: リクエストボディからuserIdを受け取らず、Authorization headerから認証トークンを検証
- **Supabase Auth統合**: `lib/auth.ts`で統一された認証ヘルパーを提供

#### SSRF攻撃対策
- **URL検証**: `lib/url-validation.ts`で安全なURLのみを許可
- **プライベートIP/ローカルホストのブロック**: 10.x.x.x、192.168.x.x、127.0.0.1などをブロック
- **危険なポートのブロック**: SSH(22)、MySQL(3306)、PostgreSQL(5432)などをブロック

#### ファイルアップロードセキュリティ
- **ファイルサイズ制限**: 10MB上限
- **MIME type検証**: JPEG、PNG、GIF、WebPのみ許可
- **パストラバーサル対策**: ファイル名に`..`、`/`、`\`を含む場合は拒否

#### その他
- **エラーメッセージのサニタイズ**: 内部エラー詳細を隠蔽し、ユーザーフレンドリーなメッセージのみ返却
- **タイムアウト設定**: 外部URLフェッチに10秒のタイムアウトを設定
...
### URLまとめて追加機能
複数のURLを効率的に処理する機能：
- **自動URL抽出**: 正規表現で文章中のURLを自動検出
- **多様な入力形式**: 改行区切り、CSV形式、通常の文章に対応
- **進行状況表示**: リアルタイムで処理状況を表示
- **エラーハンドリング**: エラーが発生しても次のURLに進み、最後に成功・失敗件数を表示
